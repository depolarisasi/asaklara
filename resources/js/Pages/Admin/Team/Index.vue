  ...
  <AdminLayout title="Team Members">
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-xl font-bold font-heading">Tim</h2>
          <p class="text-sm mt-1 text-admin-text-muted">{{ members.length }} anggota tim</p>
        </div>
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="member in members" :key="member.id"
             class="rounded-2xl p-5 flex items-center gap-4 bg-admin-card border border-admin-border">
          <img :src="member.image_url" :alt="member.name"
               class="w-16 h-16 rounded-full object-cover flex-shrink-0" />
          <div class="flex-1 min-w-0">
            <p class="font-semibold truncate">{{ member.name }}</p>
            <p class="text-sm mt-0.5 truncate text-admin-text-dim">{{ member.role }}</p>
            <span class="text-xs px-2 py-0.5 rounded-full mt-1 inline-block"
                  :class="member.active ? 'bg-green-500/15 text-green-400' : 'bg-gray-500/15 text-admin-text-dim'">
          ...
          <div class="flex flex-col gap-1">
            ...
            <button @click="confirmDelete(member)"
                    class="px-3 py-1.5 rounded-lg text-xs text-red-400 hover:bg-red-400/10 transition-colors">
              Hapus
            </button>
          </div>
        </div>

        <div v-if="members.length === 0" class="col-span-full py-16 text-center text-admin-text-muted">
