...
<section class="pt-32 pb-16 lg:pt-40 lg:pb-20">
    <div class="container mx-auto px-6 lg:px-8">
        <div class="max-w-4xl">
            ...
            <h1 class="font-heading text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-balance text-[rgb(var(--pub-fg))]">Let's Build Something Mature.</h1>
    ...
    <div class="container mx-auto px-6 lg:px-8">
        <div class="grid lg:grid-cols-5 gap-12 lg:gap-16">
            ...
            <div class="lg:col-span-2">
                <h2 class="font-heading text-2xl font-bold mb-8 text-[rgb(var(--pub-fg))]">Get in Touch</h2>

                <div class="space-y-6 mb-10">
                    @if($contact['contact.email'] ?? null)
                    <a href="mailto:{{ $contact['contact.email'] }}" class="flex items-start gap-4 group">
                        <div class="w-12 h-12 rounded-2xl bg-[rgba(var(--pub-bg),0.60)] backdrop-blur-xl border border-[rgba(var(--pub-border),0.50)] flex items-center justify-center flex-shrink-0 transition-colors group-hover:bg-primary group-hover:border-primary">
                            <svg class="w-5 h-5 text-primary group-hover:text-[rgb(var(--pub-primary-fg))] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        </div>
                        <div>
                            <p class="font-medium mb-1">Email</p>
                            <p class="text-sm text-[rgb(var(--pub-muted-fg))]">{{ $contact['contact.email'] }}</p>
                        </div>
                    </a>
                    @endif
                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-12 h-12 rounded-2xl bg-[rgba(var(--pub-bg),0.60)] backdrop-blur-xl border border-[rgba(var(--pub-border),0.50)] flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-[rgb(var(--pub-muted-fg))]">{{ $contact['contact.website'] }}</p>
                        </div>
                    </div>
                    @endif
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ $social['social.instagram'] }}" target="_blank"
                           class="w-11 h-11 rounded-full bg-[rgba(var(--pub-bg),0.60)] backdrop-blur-xl border border-[rgba(var(--pub-border),0.50)] flex items-center justify-center transition-colors hover:bg-primary hover:border-primary hover:text-[rgb(var(--pub-primary-fg))] text-[rgb(var(--pub-muted-fg))]"
            ...
            <div class="lg:col-span-3">
                <div class="p-8 lg:p-10 rounded-3xl bg-[rgba(var(--pub-bg),0.70)] backdrop-blur-xl border border-[rgba(var(--pub-border),0.50)]">
                    <h2 class="font-heading text-2xl font-bold mb-2 text-[rgb(var(--pub-fg))]">Send Us a Message</h2>
                    <p class="mb-8 text-[rgb(var(--pub-muted-fg))]">Fill out the form below and we'll get back to you as soon as possible.</p>

                    @if(session('success'))
                    <div class="mb-6 p-4 rounded-xl bg-[rgba(var(--pub-primary),0.1)] border border-[rgba(var(--pub-primary),0.2)]">
                    ...
                    <form action="{{ route('contact.submit') }}" method="POST" class="space-y-6"
                        ...
                        <div class="grid sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" required
                                       ...
                                       class="w-full rounded-xl h-12 px-4 text-sm transition-all focus:outline-none bg-[rgba(var(--pub-card),0.5)] border border-[rgba(var(--pub-border),0.5)] text-[rgb(var(--pub-fg))] focus:border-primary">
                        ...
                        <div>
                            ...
                            <textarea name="message" rows="6" required
                                      ...
                                      class="w-full rounded-xl px-4 py-3 text-sm transition-all focus:outline-none resize-none bg-[rgba(var(--pub-card),0.5)] border border-[rgba(var(--pub-border),0.5)] text-[rgb(var(--pub-fg))] focus:border-primary">{{ old('message') }}</textarea>
                            @error('message')<p class="mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                        </div>

                        <button type="submit"
                                :disabled="sending"
                                class="w-full py-4 rounded-full font-semibold flex items-center justify-center gap-2 transition-opacity hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed bg-primary text-[rgb(var(--pub-primary-fg))]">
                            {{-- Loading state --}}
